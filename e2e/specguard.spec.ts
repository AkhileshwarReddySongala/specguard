...
test("shows coverage controls and streamed AI progress", async ({ page }) => {
  await mockAnalysis(page); await page.goto("/"); await page.getByRole("button", { name: "COMPILE CONTRACT" }).click(); await expect(page.getByText("AI JUDGMENT COVERAGE")).toBeVisible(); await page.getByLabel("Judge all rules").check(); await page.getByRole("button", { name: "ANALYZE PULL REQUEST" }).click(); await expect(page.getByText("AI coverage 1/1 selected")).toBeVisible();
});
