...
export const compiledCheckSchema = z.object({ id: z.string().min(1), requirementQuote: z.string().min(1), specLine: z.number().int().positive(), level: severitySchema, mode: enforcementModeSchema, target: z.string(), rationale: z.string() });
export const compiledContractSchema = z.object({ checks: z.array(compiledCheckSchema).max(30), unexpressibleRules: z.array(z.string()).max(30), compiler: compilerSchema });
...
export type CompiledCheck = z.infer<typeof compiledCheckSchema>; export type CompiledContract = z.infer<typeof compiledContractSchema>; export type PRSnapshot = z.infer<typeof prSnapshotSchema>; export type Finding = z.infer<typeof findingSchema>; export type Verdict = z.infer<typeof verdictSchema>; export type AnalysisResult = z.infer<typeof analysisResultSchema>;