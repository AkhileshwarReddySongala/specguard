...
export const judgmentModeSchema = z.enum(["relevant", "all"]);
...
export const providerDiagnosticSchema = z.enum(["timeout", "gateway_timeout", "rate_limited", "invalid_output", "cooldown", "cancelled"]);
export const providerStatusSchema = z.enum(["nvidia", "gemini", "ollama", "deterministic-only", "fallback", "mixed"]);
...
export const judgmentCoverageSchema = z.object({
  mode: judgmentModeSchema,
  totalRules: z.number().int().nonnegative(),
  scopeExcludedRules: z.number().int().nonnegative(),
  selectedRules: z.number().int().nonnegative(),
  completedRules: z.number().int().nonnegative(),
  unassessedRules: z.number().int().nonnegative(),
  complete: z.boolean(),
  providersUsed: z.array(providerStatusSchema).max(3),
});
export const analysisResultSchema = z.object({
  snapshot: prSnapshotSchema,
  contract: compiledContractSchema,
  ...
  judgmentCoverage: judgmentCoverageSchema,
});
export const analyzeRequestSchema = z.object({ prUrl: z.string().min(1).max(2_048), compiledContract: compiledContractSchema, judgmentMode: judgmentModeSchema.default("relevant") });
...
export type JudgmentCoverage = z.infer<typeof judgmentCoverageSchema>;
