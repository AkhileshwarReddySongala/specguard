...
describe("compile route", () => {
  it("returns one validated final contract for a large mocked NVIDIA compile", async () => {
    ...
    const rules = Array.from({ length: 40 }, (_, index) => `- Review rule ${index}.`).join("\n"); const specMarkdown = `${rules}\n${Array.from({ length: 2_200 }, () => "# padding").join("\n")}`;
    expect(specMarkdown.length).toBeGreaterThan(22_000); const response = await POST(new Request("http://localhost/api/compile", { method: "POST", body: JSON.stringify({ specMarkdown }), headers: { "Content-Type": "application/json" } }));
